/**
 * views/config.js — includes Empresas/Dependencias management
 */
App.cfg={
  async load(){
    try{
      ...
      // Update empresa selects in user modals dynamically
  ...
  async savePesos(){
    ...
    try{const r=await api.savePesos(d,e);if(r.success)toast('Guardado','ok');else toast(r.message,'err')}catch(ex){toast(ex.message,'err')}
  },
  _catData:{},
  async loadCatArea(){
    ...
    try{
      ...
      ct.innerHTML=Object.keys(catsToRender).map(cat=>{
        ...
        return '<div style="margin-bottom:14px;padding:12px;border:1px solid var(--s2);border-radius:10px;background:var(--s0)"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong style="font-size:.85rem">'+esc(cat)+'</strong><span style="font-size:.68rem;color:var(--s4)">'+pregs.length+'</span></div><div id="'+safeId+'">'+pregs.map(p=>'<div class="adm-it"><input class="adm-in cq-in" data-cat="'+esc(cat)+'" value="'+esc(p)+'"><button class="bd-sm" onclick="this.parentElement.remove()">🗑</button></div>').join('')+'</div><button class="btn ba" style="font-size:.7rem;padding:4px 10px;margin-top:6px" onclick="App.cfg.addQuestion(\''+safeId+'\',\''+esc(cat)+'\')">+ Pregunta</button></div>';
      }).join('')+'<div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn ba" onclick="App.cfg.addCategory()">+ Categoría</button><button class="btn bp" onclick="App.cfg.saveCatArea()">💾 Guardar</button></div>';
  ...
  async saveCatArea(){
    ...
    try{const r=await api.saveCategoriasDiarias(area,datos);if(r.success){toast(datos.length+' preguntas guardadas','ok');this.loadCatArea()}else toast(r.message,'err')}catch(e){toast(e.message,'err')}
  },
  async loadParamsArea(){
    ...
    try{
      ...
      ct.innerHTML=areas.slice().sort().map(a=>{const ap=pa[a]||[];const has=ap.length>0;return '<div style="margin-bottom:12px;padding:12px;border:1px solid var(--s2);border-radius:10px;background:var(--s0)"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong style="font-size:.85rem">'+esc(a)+'</strong></div><div id="pa-'+a.replace(/\s/g,'_')+'">'+(has?ap:gp).map(p=>'<div class="adm-it"><input class="adm-in pa-in" data-area="'+esc(a)+'" value="'+esc(p)+'"><button class="bd-sm" onclick="this.parentElement.remove()">🗑</button></div>').join('')+'</div><div style="display:flex;gap:6px;margin-top:6px"><button class="btn ba" style="font-size:.7rem;padding:4px 10px" onclick="App.cfg.addPA(\''+esc(a)+'\')">+</button><button class="btn bp" style="font-size:.7rem;padding:4px 10px" onclick="App.cfg.savePA(\''+esc(a)+'\')">💾</button></div></div>'}).join('')||'<div class="empty"><div class="empty-t">No hay áreas</div></div>';
    }catch(e){toast(e.message,'err')}
  },
  ...
  async savePA(a){const v=Array.from(document.querySelectorAll('.pa-in[data-area="'+a+'"]')).map(i=>i.value.trim()).filter(Boolean);try{const r=await api.saveParametrosArea(a,v);if(r.success)toast('Guardado','ok');else toast(r.message,'err')}catch(e){toast(e.message,'err')}},
  lists:{
    ...
    _r(id,arr,k){var el=document.getElementById(id);if(!el)return;el.innerHTML=(arr||[]).map((v,i)=>'<div class="adm-it"><input class="adm-in ai-'+k+'" value="'+esc(v)+'"><button class="bd-sm" onclick="App.cfg.lists.rm(\''+k+'\','+i+')">🗑</button></div>').join('')},
    ...
    async save(k){const map={p:'saveParametros',ps:'saveParametrosSup',ar:'saveAreas',se:'saveSedes',em:'saveEmpresas'};var fn=map[k];if(!fn)return;try{await api[fn](this._v(k));ADM[k]=this._v(k);toast('Guardado','ok')}catch(e){toast(e.message,'err')}}
  }
};