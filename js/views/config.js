/**
 * views/config.js — Weights, daily categories/questions per area, params, areas, sedes
 */
App.cfg={
  async load(){
    try{
  ...
  // ── Daily Categories per Area (FIX: properly collects all categories) ──
  _catData:{},
  async loadCatArea(){
    ...
    try{
      ...
      // If no categories for this area, start with defaults
      ...
      ct.innerHTML=Object.keys(catsToRender).map(cat=>{
        const pregs=catsToRender[cat]||[];
        const safeId='cq-'+cat.replace(/[^a-zA-Z0-9]/g,'_');
        return '<div style="margin-bottom:14px;padding:12px;border:1px solid var(--s2);border-radius:10px;background:var(--s0)">'
          ...
          +'<strong style="font-size:.85rem">'+esc(cat)+'</strong>'
          +'<span style="font-size:.68rem;color:var(--s4)">'+pregs.length+' preguntas</span></div>'
          +'<div id="'+safeId+'">'
          +pregs.map(p=>'<div class="adm-it"><input class="adm-in cq-in" data-cat="'+esc(cat)+'" value="'+esc(p)+'"><button class="bd-sm" onclick="this.parentElement.remove()">🗑</button></div>').join('')
          +'</div>'
          +'<button class="btn ba" style="font-size:.7rem;padding:4px 10px;margin-top:6px" onclick="App.cfg.addQuestion(\''+safeId+'\',\''+esc(cat)+'\')">+ Pregunta</button>'
          +'</div>';
      }).join('')
      +'<div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap">'
      +'<button class="btn ba" onclick="App.cfg.addCategory()">+ Nueva categoría</button>'
      +'<button class="btn bp" onclick="App.cfg.saveCatArea()">💾 Guardar categorías</button>'
      +'</div>';
    }catch(e){ct.innerHTML='<div class="empty">'+e.message+'</div>'}
  },

  addQuestion(containerId,cat){
    const ct=$(containerId);if(!ct)return;
    const d=document.createElement('div');d.className='adm-it';
    d.innerHTML='<input class="adm-in cq-in" data-cat="'+esc(cat)+'" value=""><button class="bd-sm" onclick="this.parentElement.remove()">🗑</button>';
    ct.appendChild(d);
    // Focus the new input
  ...
  addCategory(){
    ...
    const ct=$('catAreaCont');if(!ct)return;
    const safeId='cq-'+cat.replace(/[^a-zA-Z0-9]/g,'_');
    // Check if container already has a buttons div at the end
    ...
    div.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong style="font-size:.85rem">'+esc(cat)+'</strong></div>'
      +'<div id="'+safeId+'"><div class="adm-it"><input class="adm-in cq-in" data-cat="'+esc(cat)+'" value=""><button class="bd-sm" onclick="this.parentElement.remove()">🗑</button></div></div>'
      +'<button class="btn ba" style="font-size:.7rem;padding:4px 10px;margin-top:6px" onclick="App.cfg.addQuestion(\''+safeId+'\',\''+esc(cat)+'\')">+ Pregunta</button>';
    if(btnsDiv)ct.insertBefore(div,btnsDiv);else ct.appendChild(div);
  },

  async saveCatArea(){
    const area=$('catAreaSel').value;
    if(!area){toast('Seleccioná un área','err');return}
    ...
    try{
      const r=await api.saveCategoriasDiarias(area,datos);
      if(r.success){toast('Categorías guardadas para '+area+' ('+datos.length+' preguntas)','ok');this.loadCatArea()}
    ...
    try{
      ...
      ct.innerHTML=areas.map(a=>{
        ...
        return '<div style="margin-bottom:12px;padding:12px;border:1px solid var(--s2);border-radius:10px;background:var(--s0)"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><strong style="font-size:.85rem">'+esc(a)+'</strong><span style="font-size:.68rem;color:var(--s4)">'+(has?ap.length+' personalizados':'Globales ('+gp.length+')')+'</span></div>'
          ...
          +'<div style="display:flex;gap:6px;margin-top:6px"><button class="btn ba" style="font-size:.7rem;padding:4px 10px" onclick="App.cfg.addPA(\''+esc(a)+'\')">+</button><button class="btn bp" style="font-size:.7rem;padding:4px 10px" onclick="App.cfg.savePA(\''+esc(a)+'\')">💾</button><button class="btn bo" style="font-size:.7rem;padding:4px 10px" onclick="App.cfg.resetPA(\''+esc(a)+'\')">↺ Globales</button></div></div>';
  ...
  // ── Admin lists (global params, areas, sedes) ──
  lists:{
