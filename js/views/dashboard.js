/**
 * views/dashboard.js — Combined dashboard: daily + election = final score
 */
(function(){
  let CHS={};
  App.dash={
    async load(){
      try{
        // Get combined data (daily + election + final)
        ...
        const conDiaria=resultados.filter(r=>r.promDiaria>0).length;
        const conElec=resultados.filter(r=>r.promEleccion>0).length;
        ...
        $('dSt').innerHTML=
          ...
          '<div class="st sa"><div class="st-i">📋</div><div class="st-n">'+conDiaria+'</div><div class="st-l">Eval. diaria</div></div>'+
        ...
        ).join('')||'<div class="empty"><div class="empty-t">Sin datos aún</div></div>';

        // Top per sede - build from resultados
        const topSede={};
        resultados.forEach(r=>{
          if(r.puntajeFinal>0&&r.sede&&(!topSede[r.sede]||r.puntajeFinal>topSede[r.sede].puntajeFinal)){
            topSede[r.sede]=r;
          }
        });
        ...
        $('tRank').innerHTML='<thead><tr><th>#</th><th>Colaborador</th><th>Área</th>'+
          '<th>Diaria ('+pesos.pesoDiaria+'%)</th>'+
          '<th>Elección ('+pesos.pesoEleccion+'%)</th>'+
          ...
          }).join(''):'<tr><td colspan="6" style="text-align:center;padding:20px;color:var(--s4)">Sin evaluaciones aún</td></tr>')+
          '</tbody>';

        // Bar chart — final scores
        const chartData=ranked.slice(0,20); // top 20 for readability
        ...
        if(c1){
          ...
          CHS.d=new Chart(c1,{
            type:'bar',
            data:{
              labels:chartData.map(r=>r.nombre.split(' ')[0]),
              datasets:[
        ...
        // Trend line from old dashboard endpoint (still useful)
        try{
