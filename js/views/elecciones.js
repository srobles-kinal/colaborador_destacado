...
App.elec={
  ...
  async crear(){const n=prompt('Nombre de la elección (ej: Marzo 2026):');if(!n)return;try{const r=await api.crearEleccion({nombre:n});if(r.success){toast('Creada','ok');this.load();syncData()}else toast(r.message,'err')}catch(e){toast(e.message,'err')}},
  async activar(id){if(!confirm('¿Activar? La anterior se cerrará.'))return;try{const r=await api.activarEleccion(id);if(r.success){toast('Activada','ok');this.load();syncData()}else toast(r.message,'err')}catch(e){toast(e.message,'err')}},
  async cerrar(id){if(!confirm('¿Cerrar esta elección?'))return;try{const r=await api.cerrarEleccion(id);if(r.success){toast('Cerrada','ok');this.load();syncData()}else toast(r.message,'err')}catch(e){toast(e.message,'err')}}
};
