/**
 * views/usuarios.js — User CRUD, edit modal, reset pwd
 */
(function(){
  let _users=[], _editingEmail='';

  function popSelects(aId,sId){
    const areas=DATA?.areas||ADM.ar||[],sedes=DATA?.sedes||ADM.se||[];
    const a=$(aId),s=$(sId);
  ...
  App.usr={
    ...
    edit(email){
      ...
      _editingEmail=email;
      $('edEmail').textContent=email;$('edNombre').value=u.nombre||'';$('edRol').value=u.rol||'votante';$('edActivo').value=u.activo?'true':'false';
    ...
    async saveEdit(){
      ...
      const d={email:_editingEmail,nombre:$('edNombre').value.trim(),rol:$('edRol').value,area:$('edArea').value,sede:$('edSede').value,activo:$('edActivo').value==='true',permisos:perms,foto:$('edFoto')?$('edFoto').value.trim():undefined};
